...
def run_case():
    print('Case: EasyTide example with mGvM model 4')
    ...
    t0 = time()
    results = simple.mgvm_vi.inference_model_4_opt(xin, config)
    tf = time()
    ...
    fig.savefig('./results/tides_model4_mgvm.svg')
    np.save('./results/tides_model4_mgvm', (results, config))
    # fig.show()
