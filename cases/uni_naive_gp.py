...
def run_case():
    ...
    # x_v = np.array([+0.10, +0.27, +0.42, +0.48, +0.50, +0.55, +0.63, +0.65, +0.70, +0.95])
    # y_v = np.array([+0.42, +3.45, +1.67, +0.00, -1.67, +2.41, +0.10, +2.30, -3.01, +0.52])
    x_v = x
    y_v = y
    y_t = uc.cfix(y_t)
    y_v = uc.cfix(y_v)
    ...
    # plt.imshow(p, aspect='auto', origin='lower', cmap=cm.get_cmap('Oranges'))
    # plt.yticks([0, 250, 500, 750, 1000], ['$-\pi$', '$-\pi/2$', '$0$', '$+\pi/2$', '$+\pi$'])
    ...
    fig = plot.gcf()
    fig.savefig('../results/uni_naive_gp.pdf')
    np.save('../results/uni_naive_gp', config)
    plot.show()
