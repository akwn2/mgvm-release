...
def run_case():
    ...
    k1 = np.log(np.random.rand(1, 1) * 1E-4)
    k2 = np.log(np.random.rand(1, 1) * 1000)
    ...
    t0 = time()
    results = simple.mgvm_vi.inference_model_3_opt(xin, config)
    tf = time()
    ...
    p, th, mode_1, mode_2 = simple.mgvm_vi.predictive_dist(n_pred, new_mf_k1, new_mf_m1, new_k1, new_k2, res=1000)
    fig = plot.tides(th, p, y_t, y_v, pid_t, pid_v)

    fig.savefig('../results/tides_model3_mgvm.pdf')
    np.save('../results/tides_model3_mgvm', (results, config))
    # fig.show()
