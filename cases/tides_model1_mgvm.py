...
def run_case():
    print('Case: EasyTide example with mGvM model 1')
    ...
    p, th, mode = simple.mgvm_vi.predictive_dist(n_pred, new_mf_k1, new_mf_m1, new_k1, res=1000)
    fig = plot.tides(th, p, y_t, y_v, pid_t, pid_v)

    fig.savefig('../results/tides_model1_mgvm.pdf')
    np.save('../results/tides_model1_mgvm', (results, config))
    # fig.show()
