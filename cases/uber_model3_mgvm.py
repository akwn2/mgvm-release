...
def run_case():
    ...
    k1 = np.log(np.random.rand(1, 1) * 1)
    k2 = np.log(np.random.rand(1, 1) * 4)
    ...
    p, th = simple.mgvm_vi.predictive_dist(n_pred, new_mf_k1, new_mf_m1, new_k1, new_k2, res=1000)
    ...
    np.save('./results/uber_model3_mgvm', (results, config))
    # fig.show()
