...
def run_case():
    ...
    k1 = np.log(np.random.rand(1, 1) * 1E-2)
    ...
    p, th = simple.mgvm_vi.predictive_dist(n_pred, new_mf_k1, new_mf_m1, k1, 0.)
    ...
    for ii in xrange(0, y_v.shape[0]):
        holl_score += uc.holl(y_v[ii], new_psi_p[ii], 0, new_k1, 0)

    print 'HOLL Score: ' + str(holl_score)

    fig.savefig('./results/uni_model1_mgvm.pdf')
    np.save('./results/uni_model1_mgvm', (results, config))
    plot.show()
